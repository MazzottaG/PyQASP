...
void Enumeration::solve() {
    assert(DLV2::Options::enumerationStrategy == ENUMERATION_BC || DLV2::Options::enumerationStrategy == ENUMERATION_BT);
    if(DLV2::Options::enumerationStrategy == ENUMERATION_BC)
        enumerationBlockingClause();
    else
        enumerationBacktracking();
    
    if(numberOfModels == 0) { trace_msg(enumeration, 1, "No model found."); solver.printIncoherence(); return; }
}
...
bool Enumeration::foundModel() {
    ...
    if(++numberOfModels >= DLV2::Options::maxModels) { trace_msg( enumeration, 1, "Enumerated " << DLV2::Options::maxModels << "." ); return false; }    
    return true;
}
