    ...
    string scriptDirectory ) : Propagator()
{
    ...
    if( interpr == PYTHON_INTERPRETER )
        interpreter = new MyPythonInterpreter( fn, scriptDirectory, DLV2::Options::callPyFinalize );
    else if( interpr == PERL_INTERPRETER )
        interpreter = new MyPerlInterpreter( fn, scriptDirectory );
    else
        DLV2::ErrorMessage::errorGeneric( "Unsupported interpreter" );
    ...
    Solver& solver )
{
    if( interpreter->checkMethod( method_plugins_getLiterals ) )
    {
        ...
        for( unsigned int i = 0; i < output.size(); i++ )
        {
            ...
            Literal lit = Literal::int2Lit( output[ i ] );
    ...
    const string& method )
{
   if( !interpreter->checkMethod( method ) )
        DLV2::ErrorMessage::errorGeneric( "Method " + method + " is required." ); 
}
...
ExternalPropagator::checkWellFormed()
{
    if( check_endPropagation && !check_onLitTrue )
        DLV2::ErrorMessage::errorGeneric( "If " + string( method_plugins_endPropagation ) + " is used then method " + string( method_plugins_onLitTrue ) + " is required." );    
    
    if( check_endPropagation && check_onLitsTrue )
        DLV2::ErrorMessage::errorGeneric( "If " + string( method_plugins_endPropagation ) + " is used then method " + string( method_plugins_onLitsTrue ) + " cannot be used." );    
    ...
    if( ( check_onLitTrue || check_onLitsTrue ) && 
        ...
        DLV2::ErrorMessage::errorGeneric( "If " + string( method_plugins_onLitTrue ) + " or " + string( method_plugins_onLitsTrue ) + " are used then " + 
    ...
    if( interpreter->checkMethod( method_plugins_getReason ) && interpreter->checkMethod( method_plugins_getReasonForLiteral ) )
        DLV2::ErrorMessage::errorGeneric( "Exactly one method between " + string( method_plugins_getReason ) + " and " + string( method_plugins_getReasonForLiteral ) + " can be used." );
        
    if( ( check_checkAnswerSet && !check_getReasonForCheckFailure ) || ( !check_checkAnswerSet && check_getReasonForCheckFailure ) )
        if( !check_checkPartialInterpretation )
            DLV2::ErrorMessage::errorGeneric( "Method " + string( method_plugins_getReasonsForCheckFailure ) + " is required when " +  string( method_plugins_checkAnswerSet ) + " is used." );
    ...
    if( ( check_checkPartialInterpretation && !check_getReasonForCheckFailure ) || ( !check_checkPartialInterpretation && check_getReasonForCheckFailure ) )
        if( !check_checkAnswerSet )
            DLV2::ErrorMessage::errorGeneric( "Method " + string( method_plugins_getReasonsForCheckFailure ) + " is required when " +  string( method_plugins_checkPartialInterpretation ) + " is used." );        
}
    ...
    Literal currentLiteral )
{
    ...
    {
        if( solver.getCurrentDecisionLevel() != 0 )
            DLV2::ErrorMessage::errorGeneric( "Reason is not well-formed: At least one of the literals in the reason must be inferred at the current decision level." );
        else
            return NULL;
    }
    ...
    {
        checkIdOfLiteral( solver, output[ i ] );
        Literal l = Literal::int2Lit( output[ i ] );
        if( solver.isUndefined( l ) )
            DLV2::ErrorMessage::errorGeneric( "Reason is not well-formed: Literal with id " + to_string( l.getId() ) + " is undefined." );
        if( solver.isTrue( l ) )
            DLV2::ErrorMessage::errorGeneric( "Reason is not well-formed: Literal with id " + to_string( l.getId() ) + " is true." );
    ...
    Solver& solver )
{
    ...
    if( output.size() <= 2 || output[ 0 ] != 0 || output.back() != 0 )
        DLV2::ErrorMessage::errorGeneric( "Reasons must start and terminate with 0." );
    ...
    Solver& solver )
{
    if( check_simplifyAtLevelZero )
    {
        ...
        {
            checkIdOfLiteral( solver, output[ i ] );
            if( !solver.addClause( Literal::int2Lit( output[ i ] ) ) )
                break;
        }
    }
}
    ...
    int id ) const
{
    ...
    if( var == 0 || var > solver.numberOfVariables() )
        DLV2::ErrorMessage::errorGeneric( "Variable " + to_string( var ) + " does not exist." );
    if( solver.hasBeenEliminated( var ) )
        DLV2::ErrorMessage::errorGeneric( "Variable " + to_string( var ) + " has been removed from initial simplifications. Use " + method_plugins_getVariablesToFreeze + " to prevent the elimination of the variables." );
}
    ...
    Literal conflictLiteral )
{
    ...
    else
    {
        assert( solver.getCurrentDecisionLevel() == 0 );
        delete clause;
        solver.assignLiteral( Literal::int2Lit( 1 ) );
        return;
    }
    ...
    const vector< int >& output )
{
    ...
    {
        checkIdOfLiteral( solver, output[ i ] );
        conflictLiteral = Literal::int2Lit( output[ i ] );
