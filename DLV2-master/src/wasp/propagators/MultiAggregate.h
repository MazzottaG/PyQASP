...
class MultiAggregate : public Propagator, public Reason
{
    ...
    public:        
        ...
        inline void finalize(Solver& solver, bool addBinaryClauses_)  { 
            sort();
            addBound(Literal(1,POSITIVE), UINT64_MAX);
            if(addBinaryClauses_) addBinaryClauses(solver);
            assert(checkUndefined(solver));
        }                        
    ...
    private:
        ...
        void checkTrueInference(Solver& solver, Literal lits);
        ...
        inline unsigned int hasBound( uint64_t bound ) {
            for(unsigned int i=0; i < bounds.size(); i++) if(bounds[i] == bound) return i;
            return UINT_MAX;
        }
        #endif
