...
Aggregate::reset(
    const Solver& solver )
{    
    trace_msg( waspaggregates, 2, "Calling reset for aggregate " << *this );        
    ...
    do
    {
        ...
        if( trail.empty() )
        {
            trace_msg( waspaggregates, 3, "Restored counters. Values: " << counterW1 << " - " << counterW2 );
            return;
        }
    ...
    PropagatorData p )
{
    ...
    trace_msg( waspaggregates, 10, "Aggregate: " << *this << ". Literal: " << currentLiteral.getOppositeLiteral() << " is true. Position: " << position );
    ...
    {        
        trace_msg( waspaggregates, 2, "Return. AggrLiteral: " << aggrLiteral << " - Active: " << active << " - Ac: " << ac );
        return false;
    }
    ...
    trace_msg( waspaggregates, 2, "Updating counter. Old value: " << counter << " - New value: " << counter - weights[ index ] );
    ...
    if( counter < ( int64_t ) weights[ index ] )
    {
        ...
        trace_msg( waspaggregates, 3, "A conflict happened." );        
    ...
    trace_msg( waspaggregates, 2, "Umax: " << umax << " - size: " << size() );
    while( umax < literals.size() && ( int64_t ) weights[ umax ] > counter )
    {
        if( watched[ umax ] )
        {
            ...
            {                
                ...
                trace_msg( waspaggregates, 9, "Inferring " << lit << " as true" );
//                createClauseFromTrail( lit );
            ...
            {
                trace_msg( waspaggregates, 9, "Skipping true literal " << lit );
            }
        }
        
        ++umax;
        trace_msg( waspaggregates, 3, "Updated umax. New Value: " << umax );        
    }
...
Aggregate::updateBound(
    ...
    uint64_t bound )
{
    trace_msg( waspaggregates, 1, "Updating bound. New value: " << bound );
    ...
    trace_msg( waspaggregates, 1, "Sum of weights: " << sumOfWeights );    
    ...
    trace_msg( waspaggregates, 1, "Counters: " << counterW1 << "," << counterW2 );
    weights[ 1 ] = w;
...
//            counterW2 -= weights[ i ];
    ...
    trace_msg( waspaggregates, 1, "Counters final: " << counterW1 << "," << counterW2 );
