...
{    
    public:
        ...
        void processRule(int bodyLit, const vector<int>& headRec, const vector<int>& headNonRec, const vector<int>& bodyRec, const vector<int>& bodyNonRec);
        void processComponentBeforeStarting();

    private:
        
        void addClauseToChecker(vector<Literal>& clause, unsigned int headSize);
        ...
        inline Var getCheckerFalseVar(Var v) const { assert(v < generatorToCheckerId.size() && generatorToCheckerId[v] > 0); /*Can be UINT_MAX;*/ return generatorToCheckerId[ v ]; }
        
        inline bool addLiteralInClause(Literal lit, vector<Literal>& clause) {            
            if(checker.isTrue(lit)) return false;            
            if(!checker.isFalse(lit)) clause.push_back(lit);
            return true;
