...
void CautiousReasoning::solve() {
    ...
    switch(DLV2::Options::queryAlgorithm) {
        ...
        case CHUNK_DYNAMIC:
            ...
            if(DLV2::Options::chunkSize != UINT_MAX) param=DLV2::Options::chunkSize;
            else if(DLV2::Options::chunkPercentage != UINT_MAX) param = (candidates.size() * DLV2::Options::chunkPercentage)/100;
            
            if(param == 1) iterativeCoherenceTesting();
            else if(param >= candidates.size()) overestimateReduction();
            else chunkDynamic(param);
            }
            break;
        ...
        case ITERATIVE_COHERENCE_TESTING_PREFERENCES:
            iterativeCoherenceTestingPreferences();
            break;
            
        case PRIME_IMPLICATE:
            primeImplicate();
            break;
...
//    if(DLV2::Options::queryAlgorithm==ITERATIVE_COHERENCE_TESTING_PREFERENCES)
//        iterativeCoherenceTestingPreferences();
//    else if(DLV2::Options::queryAlgorithm==PRIME_IMPLICATE)
//        primeImplicate();
//    else
//        minimalAlgorithm();
//    coreBased(2);
...
void CautiousReasoning::addAnswer(Var v) {
    ...
    if(DLV2::Options::queryVerbosity >= 2) cout << "Certain answer: " << Literal(v,POSITIVE) << endl;    
    answers.push_back(v);
    if(DLV2::Options::queryVerbosity >= 1) cout << "["<< answers.size() << "," << candidates.size() << "]" << endl;
}

void CautiousReasoning::printCandidates() {
    if(DLV2::Options::queryVerbosity >= 1) cout << "["<< answers.size() << "," << candidates.size() << "]" << endl;
    if(DLV2::Options::queryVerbosity < 3) return;
    if(DLV2::Options::queryVerbosity == 3) { cout << "Number of possible answers: " << candidates.size() << endl; return; }
    cout << "Candidates:";
    ...
    while(!candidates.empty()) {
        ...
        while(true) {
            ...
            if(conflict.size() == 1) {            
                ...
                if(waspFacade.getSolver().isVisible(v) && find(answers.begin(), answers.end(), v) == answers.end()) addAnswer(v);
...
void CautiousReasoning::minimalAlgorithm() {
    ...
    if( DLV2::Options::queryCoreCache &&
        ( DLV2::Options::queryAlgorithm == ONE_QUERIES || DLV2::Options::queryAlgorithm == KDYN_QUERIES || DLV2::Options::queryAlgorithm == PMRES_QUERIES ) )
...
unsigned int CautiousReasoning::solveAndProcessCore(vector<Literal>& assumptions, vector<Literal>& conflict) {
    ...
    if(!exists) {
        ...
        if(conflict.size() == 1) {
            Literal lit = conflict[0];
            Var v = lit.getVariable();
            if(waspFacade.getSolver().isVisible(v)) addAnswer(v);
...
void CautiousReasoning::printAnswers() {
    if(DLV2::Options::silent==1) cout << "Number of cautious consequences: " << answers.size() << endl;
    if(DLV2::Options::silent>=1) return;
    WaspOutput& output = waspFacade.getWaspOutput();
    output.startModel();
    for(unsigned int i=0; i < answers.size(); i++) output.printVariable(answers[i], waspFacade.getAtomsTable().getName(answers[i]), true);
    output.endModel();
}
