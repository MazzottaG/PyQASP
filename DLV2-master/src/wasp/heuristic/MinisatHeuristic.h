...
class MinisatHeuristic
{
    public:
        ...
        inline void resetVariableIncrement(float value) { variableIncrement = value; }
        inline void resetVariableDecay(float value) { variableDecay = value; }
...
MinisatHeuristic::MinisatHeuristic( Solver& s ) :
    solver( s ), variableIncrement( DLV2::Options::initVariableIncrement ), variableDecay( DLV2::Options::initVariableDecay ), chosenVariable( 0 ), heap( ActivityComparator( act, vars ) ), preferredChoices( LiteralActivityComparator( act, vars ) )
{
    act.push_back( 0.0 );
