...
{
    if( filename == NULL )
        DLV2::ErrorMessage::errorGeneric( "Script file is not specified." );

    if( interpr == PYTHON_INTERPRETER )
        interpreter = new MyPythonInterpreter( filename, scriptDirectory, DLV2::Options::callPyFinalize, false );
    ...
    if( !check_choiceVars )
        DLV2::ErrorMessage::errorGeneric( "Method " + string( method_selectLiteral ) + " is mandatory." );
...
void ExternalHeuristic::selectLiteral()
{
    ...
    if( size == 0 )
        DLV2::ErrorMessage::errorGeneric( error_selectLiteral );
    ...
    if( result[ 0 ] == 0 )
    {
        ...
        {
            case DO_RESTART:                
                if( size != 2 ) DLV2::ErrorMessage::errorGeneric( error_selectLiteral );
                break;
            case FALLBACK_HEURISTIC:
            case UNROLL:                
            case CHOICE:
                if( size != 3 ) DLV2::ErrorMessage::errorGeneric( error_selectLiteral );
                value = result[ 2 ];
                break;
            default:
                DLV2::ErrorMessage::errorGeneric( error_selectLiteral );
...
void ExternalHeuristic::initFallback()
{
    if(interpreter->checkMethod(method_getPreferences)) {
        ...
        for(int i = output.size()-1; i >= 0; i--) {
            ...
            if(v > solver.numberOfVariables())
                DLV2::ErrorMessage::errorGeneric( "Variable " + to_string(v) + " does not exist." );
            if(!solver.isUndefined(v)) continue;
            prefs.push_back(Literal(lit));
        }
    ...
    if( output.size() % 2 != 0 )
        DLV2::ErrorMessage::errorGeneric( error_initfallback );
    if( output.empty() )
        return;    
...
void ExternalHeuristic::factorFallback()
{
    ...
    if( output.size() % 2 != 0 )
        DLV2::ErrorMessage::errorGeneric( error_factorfallback );
    if( output.empty() )
        return;
    for( unsigned int i = 0; i < output.size() - 1; i = i + 2 )
    {
        ...
        switch( status )
        {
            case CHOICE:
            {
                ...
                if( varChoice == 0 || varChoice > solver.numberOfVariables() )
                    DLV2::ErrorMessage::errorGeneric( "Variable " + to_string( varChoice ) + " does not exist." );

                if( !solver.hasBeenEliminated( varChoice ) )
                {
                    Literal lit = Literal::int2Lit( value );
                    if( solver.isUndefined( lit ) )
                        return lit;
                    else if( solver.isFalse( lit ) )
                        DLV2::ErrorMessage::errorGeneric( "Contradiction in the choice" );
                }
            ...
            case UNROLL:
                ...
                DLV2::ErrorMessage::errorGeneric( "Cannot unroll variable " + to_string( value ) + ", it does not exist." );            
                if( !solver.hasBeenEliminated( value ) )
                    DLV2::ErrorMessage::errorGeneric( "Cannot unroll deleted variable " + to_string( value ) + "." );
