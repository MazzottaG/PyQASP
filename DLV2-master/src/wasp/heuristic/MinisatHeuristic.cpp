...
MinisatHeuristic::simplifyVariablesAtLevelZero()
{
    ...
    if( DLV2::Options::initSign == INIT_SIGN_MINISAT_ALLTRUE )
    {
        for( unsigned int i = 0; i < undefinedVars.size(); i++ )
            setSign( undefinedVars[ i ] );
    }    
    
    if( DLV2::Options::initMinisatHeuristic == INIT_MINISAT_ALL_EQUALS && DLV2::Options::initValue == 0 ) return;
...
{
    switch( DLV2::Options::initMinisatHeuristic )
    {
        ...
        default:
            DLV2::ErrorMessage::errorGeneric( "Invalid value for initialization of minisat heuristic." );
    }
}
...
void MinisatHeuristic::initMinisatMoms( Var v, unsigned int& score )
{
    ...
    score = ( ( pos * neg ) << 10 ) + ( pos + neg ) + DLV2::Options::initValue;
    if( DLV2::Options::initSign == INIT_SIGN_MINISAT_MIXED ) setSign( pos > neg ? v : -v );
}
...
{
    if( !solver.isVisible(v) ) return;
    score = DLV2::Options::initValue;
    if( DLV2::Options::initSign == INIT_SIGN_MINISAT_MIXED ) setSign( v % 2 == 0 ? v : -v );
}
