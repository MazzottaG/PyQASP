...
namespace DLV2
{
    ...
    class ErrorMessage
    {
        public:
            ...
            static void errorSparqlImport( char* errorMessage )
			{
				std::cerr << "Sparql import error" << ": " << errorMessage << std::endl;
				exit( ERROR_GENERIC_CODE );
			}
            static void errorSparqlImport(const char* errorMessage )
			{
				std::cerr << "Sparql import error" << ": " << errorMessage << std::endl;
				exit( ERROR_GENERIC_CODE );
			}
            static void errorSparqlImport( std::string errorMessage )
			{
				std::cerr << "Sparql import error" << ": " << errorMessage << std::endl;
				exit( ERROR_GENERIC_CODE );
			}
